...
func (r *dbIf) Open() (err error) {
	rc := syscall.Syscall(_dbif_gate_dbIf_Open, 2, uintptr(unsafe.Pointer(r)),
		uintptr(24), 0)
	if rc != 0 {
...
func (r *dbIf) Close() (err error) {
	rc := syscall.Syscall(_dbif_gate_dbIf_Close, 2, uintptr(unsafe.Pointer(r)),
		uintptr(24), 0)
	if rc != 0 {
		return dbif_getError(rc)
	}
...
func (r *dbBatch) Do() (err error) {
	rc := syscall.Syscall(_dbif_gate_dbBatch_Do, 2, uintptr(unsafe.Pointer(r)),
		uintptr(24), 0)
	if rc != 0 {
		return dbif_getError(rc)
	}
